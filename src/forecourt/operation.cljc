(ns forecourt.operation
  "OperationActor -- one automotive-fuel-retail operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (ForecourtAdvisor) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the Forecourt Safety Governor (:govern) and
  ...
  One graph run = one forecourt operation (intake -> advise -> govern ->
  decide -> commit | hold | approval). No unbounded inner loop -- each
  operation is auditable and checkpointed. A fuel-sale's life is advanced
  by MANY ops (intake / verify / dispense / settle), each its own
  independent run.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human operator (forecourt operator / station manager).
  The approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:pump/dispense`/`:sale/settle` ALWAYS reach this node
  when the governor is clean -- see `forecourt.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [forecourt.forecourtadvisor :as forecourtadvisor]
...
(defn build
  ...
  `forecourt.store/Store`).
  opts:
    :advisor      -- a `forecourt.forecourtadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (forecourtadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ForecourtAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (forecourtadvisor/-advise advisor store request)]
            {:proposal p :audit [(forecourtadvisor/trace request p)]})))

      ;; Forecourt Safety Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
